...
from montecarlo.mc_ledger import MCWithdrawalLedger 
...
def build_outcome_from_ledger(
    mc_ledger: MCWithdrawalLedger,
    context: SimulationContext,
    spending_model: SpendingModel
) -> MCOutcome:

    schedule = context.schedule
    mc_ledger_df = mc_ledger.frame.df
    outcome = MCOutcomeLedger()

    for year in sorted(mc_ledger_df['year'].unique()):
        year_df = mc_ledger_df[mc_ledger_df['year'] == year]

        base_balance = year_df['base_balance'].sum()
        end_balance = year_df['end_balance'].sum()

        mc_amount = year_df['mc_amount'].sum()
        mc_return_rate = year_df['mc_return_rate'].mean()

        income_amount = year_df[year_df['mc_type'].str.startswith('inc_')]['mc_amount'].sum()

        shortfall_amount = year_df['shortfall_amount'].sum()
        synthetic_flag = year_df['synthetic_flag'].any()

        goal_met = year_df['goal_met'].any()
        rmd_met = year_df['mc_type'].str.contains('ira_rmd', case=False, na=False).any()

        outcome.add_year(
            year=year,
            age=int(year_df['age'].max()),
            sim_num=int(year_df['sim_num'].iloc[0]),
            base_balance=base_balance,
            end_balance=end_balance,
            income_amount=income_amount,
            mc_amount=mc_amount,
            mc_return_rate=mc_return_rate,
